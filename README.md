...
Shocktail features a sign-up and sign-in page as the beginning of the website. After signing into the website, the user finds an age verification popup. The login page sign-up and sign-in are authenticated via Firebase. The username and password are stored in the Firebase realtime database as well as MySQL via a user model seen here:
